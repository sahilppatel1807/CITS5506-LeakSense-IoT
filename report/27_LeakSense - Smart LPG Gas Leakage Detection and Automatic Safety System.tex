...
Things prototype that detects LPG risk, responds locally, and provides
real-time cloud visibility. The system was developed using an iterative
prototyping method and integrates an ESP32 microcontroller, a MEMS gas sensor,
...
during Wi-Fi interruption. The prototype cost approximately \$94~AUD. Key
limitations are the prototype-scale fan, the absence of an automatic gas
...
for Danger --- are prototype calibration constants. For a production safety
...
and because its small footprint suits a prototype enclosure
...
during activation, well within the prototype power budget
...
intervention. The 40\,mm form factor is appropriate for prototype
...
\subsubsection{Supporting Prototyping Components}
The remaining components support wiring, assembly, and stable testing of
the prototype. An 830 tie-point solderless breadboard with dual isolated
power rails was used as the prototyping platform because it provides
...
prototype. It identifies the ESP32 GPIO assignments, the two separate I2C
...
prototype, including the ESP32-E placement, sensor modules, OLED display,
...
\begin{figure}[H]
  ...
  \caption{LeakSense prototype breadboard circuit showing the ESP32-E,
...
Warning, 1.90\,V for Danger) are prototype calibration constants determined
...
The exhaust fan used in this prototype is a small 12\,V unit suitable for
...
development prototype. A production deployment must implement Firebase
...
\begin{table}[H]
\centering
\caption{LeakSense prototype component costs.}
\label{tab:cost}
\renewcommand{\arraystretch}{1.2}
...
of systems offering certified shutoff integration. The prototype integrates a MEMS analog gas sensor, 
...
Beyond the working prototype, the project contributes a reproducible reference architecture 
...
and water leak detection. The empirical voltage thresholds, prototype-scale fan, and absence 
...
This guide describes how to assemble, configure, and run the LeakSense
...
used by the LeakSense prototype. The same source code is also available in the
...
the same code used by the prototype.
...
\leaksenselisting[language=C++]{ESP32 firmware source code: firmware/blinking/src/main.cpp.}{lst:firmware-main}{../firmware/blinking/src/main.cpp}

\subsection{PlatformIO Configuration}

\leaksenselisting{PlatformIO configuration: firmware/blinking/platformio.ini.}{lst:platformio}{../firmware/blinking/platformio.ini}

\subsection{Credential Template}

\leaksenselisting[language=C++]{Credential template: firmware/blinking/src/secrets.h.example.}{lst:secrets-example}{../firmware/blinking/src/secrets.h.example}

\subsection{Dashboard HTML}

\leaksenselisting{Dashboard HTML: dashboard/index.html.}{lst:dashboard-html}{../dashboard/index.html}

\subsection{Dashboard CSS}

\leaksenselisting{Dashboard CSS: dashboard/css/style.css.}{lst:dashboard-css}{../dashboard/css/style.css}

\subsection{Dashboard JavaScript}

\leaksenselisting{Dashboard Firebase credential template: dashboard/js/secrets.example.js.}{lst:dashboard-secrets-example}{../dashboard/js/secrets.example.js}

\leaksenselisting{Firebase JavaScript: dashboard/js/firebase.js.}{lst:firebase-js}{../dashboard/js/firebase.js}

\leaksenselisting{Main dashboard JavaScript: dashboard/js/dashboard.js.}{lst:dashboard-js}{../dashboard/js/dashboard.js}

\leaksenselisting{Chart JavaScript: dashboard/js/charts.js.}{lst:charts-js}{../dashboard/js/charts.js}

\leaksenselisting{Mock data JavaScript: dashboard/js/mock.js.}{lst:mock-js}{../dashboard/js/mock.js}

\leaksenselisting{Notification service worker: dashboard/service-worker.js.}{lst:service-worker-js}{../dashboard/service-worker.js}
